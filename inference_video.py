...
def calculate_iou(bbox_a, bbox_b):
    ...
    calculate iou 
    args:
    - gt_bbox [array]: 1x4 single bounding box a
    - pred_bbox [array]: 1x4 single bounding box b
    returns:
    - iou [float]: iou between 2 bboxes
    """
    xmin = np.max([bbox_a[0], bbox_b[0]])
    ymin = np.max([bbox_a[1], bbox_b[1]])
    xmax = np.min([bbox_a[2], bbox_b[2]])
    ymax = np.min([bbox_a[3], bbox_b[3]])
    
    intersection = max(0, xmax - xmin) * max(0, ymax - ymin)
    bbox_a_area = (bbox_a[2] - bbox_a[0]) * (bbox_a[3] - bbox_a[1])
    bbox_b_area = (bbox_b[2] - bbox_b[0]) * (bbox_b[3] - bbox_b[1])
    
    union = bbox_a_area + bbox_b_area - intersection
...
def main(labelmap_path, model_path, tf_record_path, config_path, output_path):
    ...
    for idx, batch in enumerate(dataset):
        ...
        data = []
        for bb, sc, cl in zip(detections['detection_boxes'], detections['detection_scores'],  detections['detection_classes']):
            data.append([bb, sc, cl])
            ...
            for j, bj in enumerate(data_sorted):
                ...
                if iou > 0.5:
                    if bj[1] > bi[1]:
                        discard = True
            if not discard:
                output_bboxes.append(bi[0])
                output_scores.append(bi[1])
                output_classes.append(bi[2])
        
        detections['detection_boxes'] = np.array(output_bboxes)
        detections['detection_scores'] = np.array(output_scores)
        detections['detection_classes'] = np.array(output_classes)
